...
elements.forEach(element => {
  // Create a gradient div for the current element
  const gradientDiv = document.createElement('div');
  gradientDiv.className = 'gradient-div';

  // Append the gradient div to the current element
  element.appendChild(gradientDiv);

  // Add an event listener to update the gradient position
  element.addEventListener('mousemove', e => {
    const parentRect = element.getBoundingClientRect();
    const x = e.clientX - parentRect.left;
    const y = e.clientY - parentRect.top;

    // Update the position of the gradient div for the current element
    const gradient = element.querySelector('.gradient-div');
    gradient.style.left = `${x - 100}px`;
    gradient.style.top = `${y - 100}px`;

    // Set the opacity to 1 when the mouse is inside the div
    gradient.style.opacity = 1;
  });

  // Add an event listener to reset the opacity when the mouse exits the div
